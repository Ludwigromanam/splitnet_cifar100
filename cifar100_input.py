...
def read_cifar100(filename_queue):
  ...
  # input format.
  coarse_label_bytes = 1  # Coarse label : 0~19
  label_bytes = 1  # Original label : 0~99
  ...
  record_bytes = coarse_label_bytes + label_bytes + image_bytes
  ...
  result.coarse_label = tf.cast(
      tf.slice(record_bytes, [0], [label_bytes]), tf.int32)
  result.label = tf.cast(
      tf.slice(record_bytes, [coarse_label_bytes], [label_bytes]), tf.int32)

  # The remaining bytes after the label represent the image, which we reshape
  # from [depth * height * width] to [depth, height, width].
  depth_major = tf.reshape(tf.slice(record_bytes, [coarse_label_bytes+label_bytes], [image_bytes]),
...
def distorted_inputs(data_dir, batch_size, shuffle=True):
  ...
  padded_image = tf.image.resize_image_with_crop_or_pad(reshaped_image, height+4, width+4)
